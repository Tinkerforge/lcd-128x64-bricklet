...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Don't use device before ipcon is connected.
     ...
     let gui_tab_selected_receiver = lcd.get_gui_tab_selected_callback_receiver();
		...
		lcd.set_gui_graph_configuration(0, LCD_128X64_BRICKLET_GRAPH_TYPE_LINE, 62, 0,
                         60, 52, "X".to_string(), "Y".to_string());

		// Add a few data points (the remaining points will be 0)
		lcd.set_gui_graph_data(0,
                &[20, 40, 60, 80, 100, 120, 140, 160, 180, 200, 220, 240]);

		// Add 5 text tabs without and configure it for click and swipe without auto-redraw
		lcd.set_gui_tab_configuration(LCD_128X64_BRICKLET_CHANGE_TAB_ON_CLICK_AND_SWIPE,
                       false);
